 ...
 *
 * Запуск:
 *   node server/scripts/apply-profile-synthesis-migration.mjs
 */
...
if (!DATABASE_URL) {
    console.error('❌ DATABASE_URL не задан. Убедитесь, что переменная окружения установлена.');
    process.exit(1);
}
...
async function run() {
    ...
    const steps = [
        ...
        {
            name: 'Бэкфилл: пометить существующие записи как активные',
    ...
    if (res.rows.length !== 2) {
        console.error('❌ Не все колонки добавлены!');
        process.exit(1);
    }
